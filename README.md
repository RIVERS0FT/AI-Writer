...
当前仓库已完成基础骨架、Provider 运行时、纯文本内容持久化、章节管理、小说资料库，以及统一写作任务与 Token 统计基础：
...
每个写作任务会保存任务类型、用户要求、流水线版本、Prompt 集版本和阶段开关。每个阶段使用独立的写作步骤记录，包含状态、尝试次数、Prompt 版本、延迟、错误和 Token 汇总。

当前章节续写已经接入统一的 `draft` 步骤和请求级统计。后续阶段将继续把上下文构建、章节规划、场景生成、审查、定向改写和记忆提取迁入同一编排器。
...
模型上下文窗口是唯一的技术容量边界。只有预计输入和请求输出超过模型窗口时，系统才进行上下文兼容处理。
